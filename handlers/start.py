        ...
        if key == "ref":
            ...
            if deal:
                if deal.respondent_id == deal.creator_id:
                    return await message.answer(f"<b>{em.undov} Вы не можете присоедениться к своей сделке!</b>", reply_markup=inb.back_menu(user_id))
                if deal.respondent_id is not None:
                    return await message.answer(f"<b>{em.xxxx} Эта сделка уже занята!</b>", reply_markup=inb.back_menu(user_id))
                ...
                # Сохраняем respondent_id и переводим сделку в active
                ...
                # Уведомление creator'у: кто присоединился
                if deal.creator_role.lower() == "buyer":
                    # creator=покупатель, присоединился продавец → покупатель платит
                    await message.answer_video(cfg.video, caption=txt.deal_sign_text(
                    ...
                    # creator=продавец, присоединился покупатель → покупателю (respondent) слать pay_deal
                    ...
                    # respondent=покупатель получает инструкцию и кнопку оплаты
                    await message.answer_video(cfg.video, caption=txt.buyer_instruction(deal_id, deal.amount, deal.currency, deal.description), reply_markup=inb.pay_deal(deal.amount, deal.currency, deal_id))
                return
...
@st_router.message(Command("testerror"))
async def test_error(message: Message):
    await bot.delete_message(chat_id=message.chat.id, message_id=999999999)